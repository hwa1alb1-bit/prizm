...
Do not enable `kotlin_worker` in production. For the production launch path, prefer `DOCUMENT_EXTRACTION_PROVIDER=cloudflare-r2`; keep the legacy `PRIZM_EXTRACTION_ENGINE=kotlin_worker` flag out of production.

Do not claim full production launch readiness until service-readiness evidence also covers PRIZM statement persistence, retry behavior, and dead-letter handling for the document conversion path. Those application-level proofs are launch signoff requirements, not substitutes for the Cloudflare extractor runtime proof.

## Cloudflare R2 Production Enablement Proof

Before setting `DOCUMENT_EXTRACTION_PROVIDER=cloudflare-r2` in production, run the extractor runtime proof from staging against the container-backed Cloudflare Worker deployment, not the no-Docker Wrangler dry run.

Required extractor runtime proof evidence:
...
- Queue retry policy: confirm the `prizm-extractions` consumer has `max_retries` and `retry_delay` configured.
- Dead-letter handling: confirm the `prizm-extractions` consumer has `dead_letter_queue=prizm-extractions-dlq`.

Required full launch signoff evidence:
...
- dead-letter handling: archive DLQ evidence for an exhausted controlled failure.
